...
class SimpleTripTrackingService
{
    ...
    private function checkProximityNotifications($tripId, $trip, $stops, $currentLat, $currentLng)
    {
        ...
        // Get notification proximity from DB (assuming one setting per school/trip)
        $proximitySetting = Proximity::first();

        // Default to 50 meters if not found
        $notificationProximityMeters = $proximitySetting->notification_proximity ?? 100;

        // Convert meters to kilometers
        $proximityThreshold = $notificationProximityMeters / 1000;
