...
class CachingService
{
    ...
    public function getSchoolSettings(array|string $key = '*', $schoolID = null)
    {
        ...
        $settings = $this->schoolLevelCaching(
            config('constants.CACHE.SCHOOL.SETTINGS'),
            function () use ($schoolSettings, $schoolID) {

                $settings = $schoolSettings->builder()
                    ->where('school_id', $schoolID)
                    ->get()
                    ->pluck('data', 'name');

                // fetch school once
                $school = School::select('code')->find($schoolID);

                $settings->put('school_code', $school->code ?? '');
