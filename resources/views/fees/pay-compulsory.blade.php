            ...
            <div class="col-md-12 grid-margin stretch-card search-container">
                <div class="card">
                    ...
                    <div class="card-body d-flex justify-content-center">
                        <form class="pt-3 create-form form-validation col-sm-12 col-md-8" method="post"
                                        ...
                                        <tbody>
                                            @foreach ($fees->compulsory_fees as $compulsoryFee)
                                                <tr>
                                                    ...
                                                    <td class="text-right">
                                                        {{ $currencySymbol . ' ' . $compulsoryFee->amount }}
                                                    </td>
                                                </tr>
                                            @endforeach
                                            @if (count($fees->installments))
                                                <tr class="pay-in-installment-row">
                                                    ...
                                                    <td colspan="2" class="text-left">
                                                        <label
                                                            for="pay-in-installment-chk">{{ __('Pay in installment') }}</label>
                                                    </td>
                                                    <td class="text-right">
                                                    ...
                                                    @if (!empty($installment->is_paid))
                                                        ...
                                                        <td class="text-right">
                                                            @php
                                                                ...
                                                                if (!empty($installment->is_paid->advance_fees)) {
                                                                    foreach (
                                                                        $installment->is_paid->advance_fees
                                                                        as $advance
                                                                    ) {
                                                            ...
                                                            @endphp
                                                            <label>{{ $currencySymbol . ' ' . $installment->is_paid->amount - $advance_payment }}
                                                                <br><small>{{ $currencySymbol . ' ' . $installment->is_paid->due_charges }}</small><br>

                                                                @if (!empty($installment->is_paid->advance_fees))
                                                                    @foreach ($installment->is_paid->advance_fees as $advance)
                                                                        <br>{{ $currencySymbol . ' ' . $advance['amount'] }}
                                                                    @endforeach
                                                                @endif
                                                                <hr>
                                                                <br>{{ $currencySymbol . ' ' . $installment->is_paid->amount + $installment->is_paid->due_charges }}
                                                            </label>
                                                        </td>
                                                    @else
                                                        ...
                                                        <td colspan="2" class="text-left">
                                                            <label
                                                                for="installment-fees-{{ $key }}">{{ $installment->name }}<br>
                                                                <small
                                                                    class="{{ $installment->due_charges_amount > 0 ? 'text-danger' : 'text-success' }}">{{ __('Due date on') . ' ' . $installment->due_date }}</small>
                                                                <br><small
                                                                    class="{{ $installment->due_charges_amount > 0 ? 'text-danger' : 'text-success' }}">
                                                        ...
                                                        <td class="text-right">
                                                            ...
                                                            <span
                                                                class="final-installment-amount">{{ $currencySymbol . ' ' . round($installment->minimum_amount, 2) }}</span>
                                                            <br>
                                                            <label>
                                                                <small>{{ $currencySymbol . ' ' . round($installment->due_charges_amount, 2) }}</small><br>
                                                                <hr>
                                                                <span
                                                                    class="final-installment-amount">{{ $currencySymbol . ' ' . round($installment->total_amount, 2) }}</span>
                                                            </label>
                                                        </td>
                                                    @endif
                                                </tr>
                                            ...
                                            {{-- <tr class="installment_rows" style="display: none;">
                                                <td class="text-left"></td>
                                                <td colspan="2" class="text-left">
                                                    <label>{{ __('enter_amount') }}</label></td>
                                                <td class="justify-content-end row">
                                                    <input type="number" id="advance" name="advance" aria-label=""
                                                        class="form-control enter_amount col-6 text-right " min="0"
                                                        max="{{ $fees->remaining_amount }}"
                                                        value="{{ $fees->remaining_amount }}"
                                                        {{ !$oneInstallmentPaid ? 'disabled' : '' }}
                                            ...
                                            @if ($due_charges && !count($fees->installments))
                                                <tr>
                                                    ...
                                                    <th colspan="2">
                                                        {{ __('due_charges') }}
                                                        <span
                                                            class="text-small text-danger">({{ $fees->due_date }})</span>
                                                    </th>
                                                    <td class="text-right">{{ ' ' . $currencySymbol }}{{ $due_charges }}
                                                    </td>
                                                </tr>
                                            @endif
                                            ...
                                            <tr>
                                                ...
                                                <th class="text-right">{{ ' ' . $currencySymbol }}<span
                                                        id="total_amount_text">{{ $fees->total_compulsory_fees + $due_charges }}</span>
                                                </th>
                                            </tr>
                                            @php
                                            ...
                                            @if (!$isFullyPaid && $installment_status == 0)
                                                <tr class="without_installment_enter_amount">
                                                    <td class="text-left"></td>
                                                    <th colspan="2" class="text-left"><label>{{ __('enter_amount') }}
                                                            <span class="text-danger">*</span></label></th>
                                                    <td class="text-right"><span id="total_amount_text">
                                                            @if ($student->fees_paid)
                                                                <input type="number" name="enter_amount" min="1"
                                                                    class="form-control" {{-- max="{{ $total_compulsory_fees - $student->fees_paid->compulsory_fee_sum_amount + $due_charges }}" --}}
                                                                    id="enter_amount"
                                                                    value="{{ $total_compulsory_fees - $student->fees_paid->compulsory_fee_sum_amount + $due_charges }}"
                                                                    placeholder="{{ __('enter_amount') }}">
                                                            @else
                                                                <input type="number" name="enter_amount" min="1"
                                                                    class="form-control" {{-- max="{{ $total_compulsory_fees + $due_charges }}" --}}
                                                                    id="enter_amount"
                                                                    value="{{ $total_compulsory_fees + $due_charges }}"
                                                                    placeholder="{{ __('enter_amount') }}">
                                                            @endif
                                                    </td>
                                                </tr>
                            ...
                            @if (!$isFullyPaid)
                                            ...
                                            <div class="form-check form-check-inline">
                                                <label class="form-check-label">
                                                    <input type="radio" name="mode"
                                                        class="cash-compulsory-mode  mode" value="1" checked>
                                                    {{ __('cash') }}
                                                </label>
                                            </div>
                                            <div class="form-check form-check-inline">
                                                <label class="form-check-label">
                                                    <input type="radio" name="mode"
                                                        class="cheque-compulsory-mode mode" value="2">
                                                    {{ __('cheque') }}
                                                </label>
                                            </div>
                                        </div>
                                ...
                                <div class="form-group cheque-no-container" style="display: none">
                                    <label for="cheque_no">{{ __('cheque_no') }} <span
                                            class="text-danger">*</span></label>
                                    <input type="number" id="cheque_no" name="cheque_no"
                                        placeholder="{{ __('cheque_no') }}" class="form-control cheque-no" required />
                                </div>
