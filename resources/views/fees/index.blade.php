    ...
    <div class="content-wrapper">
        ...
        <div class="row">
            <div class="col-md-12 grid-margin stretch-card search-container">
                <div class="card">
                    <div class="card-body">
                        <form id="create-form" class="create-form common-validation-rules" action="{{ route('fees.store') }}"
                            method="POST" novalidate="novalidate" data-success-function="successFunction">
                                    ...
                                    <div class="form-inline col-md-4">
                                        ...
                                        <div class="ml-4 d-flex">
                                            ...
                                            <div class="form-check form-check-inline">
                                                <label class="form-check-label">
                                                    <input type="radio" name="include_fee_installments"
                                                        class="fees-installment-toggle user-select-none" value="0"
                                                        checked>
                                                    {{ __('Disable') }}
                                                </label>
                                            </div>
                                    ...
                                    <div data-repeater-list="fees_installments">
                                        <div data-repeater-item class="col-12 row">
                                            ...
                                            <div class="form-group col-lg-12 col-xl-3">
                                                ...
                                                {!! Form::number('due_charges', null, [
                                                    'class' => 'installment-due-charges form-control',
                                                    'placeholder' => trans('due_charges'),
                                                    'required' => true,
                                                    'data-convert' => 'number',
                                                    'min' => 0,
                                                ]) !!}
                            ...
                            <div class="border border-secondary rounded-lg mb-2 p-2 mb-3">
                                ...
                                <div class="optional-fees-types">
                                    <div data-repeater-list="optional_fees_type" class="row col-12">
                                        <div class="row col-12 mb-3" data-repeater-item>
                                            <div class="form-group col-md-12 col-lg-4">
                                                <select name="fees_type_id" id="fees_type_id"
                                                    class="form-control fees_type" aria-label="Fees Type" required>
                                                    <option value="">{{ __('Select Fees Type') }}</option>
                                                    @foreach ($feesTypeData as $feesType)
                                                        <option value="{{ $feesType->id }}">{{ $feesType->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="row" id="toolbar">
                            <div class="form-group col-sm-12 col-md-3">
                                ...
                                {!! Form::select('session_year_id', $sessionYear, $defaultSessionYear->id, [
                                    'class' => 'form-control',
                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            <th scope="col" data-field="due_charges" data-align="center">
                                                {{ __('due_charges') }} <small>(%)</small></th>
                                            <th scope="col" data-field="installments"
                                                data-formatter="feesInstallmentFormatter">{{ __('Fees Installment') }}
                                            </th>
                                            <th scope="col" data-field="fees_type" data-align="left"
                                                data-formatter="feesTypeFormatter">{{ __('Fees') }}
                                                {{ __('type') }}</th>
                                            <th scope="col" data-field="compulsory_fees" data-align="center">
                                                {{ __('Compulsory Amount') }}</th>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function handleInstallmentAmountChange(changedIndex) {
                ...
                let changedAmount = parseFloat($installments.eq(changedIndex).find('.installment-amount').val()) ||
                    0;
            ...
            $(document).on('input', '.fees-installment-repeater [data-repeater-item] .installment-amount',
                function() {
            ...
            // Optional fees — months selection limit based on number_of_months
            function initOptionalMonthsLimit($row) {
                var $numInput = $row.find('.optional-num-months');
                var $select = $row.find('.optional-months-select');
                var $hint = $row.find('.optional-months-hint');

                $numInput.on('input change', function() {
                    var limit = parseInt($(this).val()) || 0;
                    $hint.text(limit > 0 ? 'Select ' + limit + ' month(s)' : '');

                    // Deselect extras if already selected more than new limit
                    var selected = $select.find('option:selected');
                    if (selected.length > limit) {
                        selected.each(function(i) {
                            if (i >= limit) $(this).prop('selected', false);
                        });
                    }
                });

                $select.on('mousedown', function(e) {
                    var limit = parseInt($numInput.val()) || 0;
                    if (limit <= 0) {
                        e.preventDefault();
                        $numInput.addClass('is-invalid').focus();
                        setTimeout(() => $numInput.removeClass('is-invalid'), 1500);
                        return;
                    }
                    var $opt = $(e.target);
                    if ($opt.is('option') && !$opt.prop('selected')) {
                        var selectedCount = $select.find('option:selected').length;
                        if (selectedCount >= limit) {
                            e.preventDefault();
                            $hint.addClass('text-danger').text('Max ' + limit + ' month(s) allowed');
                            setTimeout(() => $hint.removeClass('text-danger').text('Select ' + limit +
                                ' month(s)'), 1500);
                        }
                    }
                });
            ...
            // Init on existing rows
            $('.optional-fees-types [data-repeater-item]').each(function() {
                initOptionalMonthsLimit($(this));
            });

            // Init on newly added rows
            $(document).on('click', '.optional-fees-types [data-repeater-create]', function() {
                setTimeout(function() {
                    var $newRow = $('.optional-fees-types [data-repeater-item]').last();
                    initOptionalMonthsLimit($newRow);
                }, 100);
            });

            $('.optional-months-select').select2('destroy'); // reset if already initialized

            $('.optional-months-select').select2({
