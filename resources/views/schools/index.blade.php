                ...
                <div class="row card-body">
                    <div class="col-12">
                        <div class="alert alert-info" role="alert">
                            ...
                            {!! __(
                            'This is a demo school. Please do not use this school for real registration. This action can only be performed once. :click_here to create the demo school.',
                            [
                            'click_here' =>
                            '<a href="javascript:void(0);" class="font-weight-bold" id="createDemoSchool">' . __('click_here') . '</a>',
                            ],
                            ) !!}
                        </div>
                    </div>
        ...
        <div class="col-lg-12 grid-margin stretch-card">
            <div class="card">
                    ...
                    <form class="create-form school-registration-form school-registration-validate"
                        ...
                        <div class="bg-light p-4 mt-4 mb-4">
                            ...
                            <div class=" col-md-6">
                                ...
                                <input type="hidden" id="lat" placeholder="Latitude" name="latitude"
                                    class="form-control mt-2">
                                <input type="hidden" id="lng" placeholder="Longitude" name="longitude"
                                    class="form-control mt-2">
                            </div>
                        </div>
                            ...
                            <div class="form-group col-sm-12 col-md-4">
                                ...
                                <div class="input-group mb-3">
                                    <input type="text" class="form-control" name="pin_code"
                                        placeholder="{{ __('pin_code') }}" required>
                                </div>
                            </div>
                        </div>
                ...
                <input class="btn btn-theme float-left ml-3 px-10 py-6" id="create-btn" type="submit"
                    value={{ __('submit') }} {{ $email_verified == 0 ? 'disabled' : '' }} style="border-radius: 4px; min-width: 150px; background: var(--theme-color); color: white; border: 1px solid var(--theme-color); margin-bottom: 5px;">
                <div class="p-4 mt-5 mb-4">
                    @if ($email_verified == 0)
                    <div class="alert alert-danger mt-2" role="alert">
                        <strong>{{ __('Warning!') }}</strong>
                        {!! __('Warning! Please configure the email settings first to continue with creating the school. :click_here', [
                        'click_here' => '<a href="/system-settings/email">' . __('click_here') . '</a>',
                        ]) !!}
                    </div>
                    @endif
                </div>
            ...
            <div class="card-body">
                ...
                <div class="row" id="toolbar">
                    <div class="form-group col-sm-12 col-md-4">
                        ...
                        {!! Form::select('package', ['' => 'All'] + $packages, null, [
                        'class' => 'form-control',
                    ...
                    <div class="">
                        <div class="col-12 text-right d-flex justify-content-end align-items-end">
                            ...
                            <a href="#" class="ml-2 table-list-type"
                                data-id="1">{{ __('Trashed') }}</a>
                        </div>
                    </div>
                </div>
                <table aria-describedby="mydesc" class='table' id='table_list' data-toggle="table"
                    data-url="{{ route('schools.show', 1) }}" data-click-to-select="true"
                    data-side-pagination="server" data-pagination="true"
                    data-page-list="[5, 10, 20, 50, 100, 200]" data-search="true" data-toolbar="#toolbar"
                    data-show-columns="true" data-show-refresh="true" data-trim-on-search="false"
                    data-mobile-responsive="true" data-sort-name="id" data-sort-order="desc"
                    data-maintain-selected="true" data-export-data-type='all'
                    data-export-options='{ "fileName": "{{ __('school') }}-<?= date(' d-m-y') ?>"
                    ...
                    <thead>
                        <tr>
                            ...
                            <th scope="col" data-field="name" data-formatter="SchoolNameFormatter">
                                {{ __('name') }}
                            </th>
                            <th scope="col" data-field="support_phone">{{ __('school') . ' ' . __('phone') }}
                            </th>
                            <th scope="col" data-field="email_verified_at"
                                data-formatter="verifyEmailStatusFormatter">{{ __('verify_email') }}</th>
                            <th scope="col" data-field="tagline" data-visible="false">{{ __('tagline') }}
                            </th>
                            <th scope="col" data-field="address">{{ __('address') }}</th>
                            <th scope="col" data-field="admin_id" data-visible="false">
                                {{ __('admin') . ' ' . __('id') }}
                            </th>
                            <th scope="col" data-field="user" data-formatter="schoolAdminFormatter">
                                {{ __('school') . ' ' . __('admin') }}
                            </th>
                            <th scope="col" data-field="active_plan">{{ __('active_plan') }}</th>
                            <th scope="col" data-field="status" data-formatter="schoolActiveStatusFormatter">
                                {{ __('status') }}
                            </th>
                            <th scope="col" data-field="operate" data-formatter="actionColumnFormatter"
                                data-events="schoolEvents" data-escape="false">{{ __('action') }}</th>
                        </tr>
                    </thead>
                </table>
            </div>
            ...
            <form id="edit-form" class="pt-3 edit-form" action="{{ url('schools') }}">
                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="form-group col-sm-12 col-md-3">
                            <label for="edit_school_support_phone">{{ __('school') . ' ' . __('phone') }} <span
                                    class="text-danger">*</span></label>
                            <input type="number" name="edit_school_support_phone" min="0"
                                id="edit_school_support_phone" placeholder="{{ __('support') . ' ' . __('phone') }}"
                                class="form-control remove-number-increment" required>
                        </div>

                        <div class="form-group col-sm-12 col-md-3" id="edit_assign_package_container">
                            <label for="assign_package">{{ __('assign_package') }} </label>
                            {!! Form::select('assign_package', $packages, null, [
                            'class' => 'form-control mb-2',
                            'placeholder' => __('select_package'),
                            'id' => 'edit_assign_package',
                            ]) !!}
                        ...
                        <div class="form-group col-sm-12 col-md-3">
                            ...
                            <input type="text" name="code" disabled id="school_code"
                                placeholder="{{ __('school_code') }}" class="form-control" required>
                    ...
                    <div class="row">
                        ...
                        <div class="form-group col-sm-12 col-md-6">
                            <div id="editMap" style="height:200px; width: 80%;"></div>
                            <input type="hidden" id="edit_lat" placeholder="Latitude" name="latitude"
                                class="form-control mt-2">
                            <input type="hidden" id="edit_lng" placeholder="Longitude" name="longitude"
                                class="form-control mt-2">
                        </div>
                    </div>
                    <div class="row">
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="form-group col-sm-12 col-md-12">
                            ...
                            <input type="email" name="edit_admin_email" id="edit-admin-email"
                                class="form-control">
                        ...
                        <div class="form-group col-sm-12 col-md-6">
                            ...
                            <input type="text" name="edit_admin_first_name" id="edit-admin-first-name"
                                placeholder="{{ __('admin') . ' ' . __('first_name') }}" class="form-control"
                                required>
                        </div>
                        <div class="form-group col-sm-12 col-md-6">
                            ...
                            <input type="text" name="edit_admin_last_name" id="edit-admin-last-name"
                                placeholder="{{ __('admin') . ' ' . __('last_name') }}" class="form-control"
                                required>
                        </div>
                        <div class="form-group col-sm-12 col-md-6">
                                ...
                                <div class="form-check w-fit-content">
                                    <label class="form-check-label ml-4">
                                        <input type="checkbox" class="form-check-input" id="manually_verify_email"
                                            name="manually_verify_email" value="1">
                                        {{ __('manually_verify_email') }}
                                    </label>
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    function initMap() {
        ...
        input.className = "map-search mt-3";
    ...
    function editInitMap() {
        ...
        // Get existing lat/lng from inputs
        let savedLat = parseFloat(document.getElementById("edit_lat").value);
        let savedLng = parseFloat(document.getElementById("edit_lng").value);
        // console.log(savedLat,savedLng);

        // Check if values exist
        const position = (!isNaN(savedLat) && !isNaN(savedLng)) ? {
            lat: savedLat,
            lng: savedLng
        } : {
            lat: 20.5937,
        ...
        map = new google.maps.Map(document.getElementById("editMap"), {
            center: position,
            zoom: (!isNaN(savedLat) ? 15 : 5), // Zoom close if editing existing location
        });
        ...
        google.maps.event.addListener(marker, 'dragend', function(event) {
            document.getElementById("edit_lat").value = event.latLng.lat().toFixed(6);
            document.getElementById("edit_lng").value = event.latLng.lng().toFixed(6);
        });
...
<script
    src="https://maps.googleapis.com/maps/api/js?key=AIzaSyBqOdT7uQebSHbnuZcqpWSYFtM8mryin4o&callback=initMap&libraries=places"
    async defer></script>
@endsection
